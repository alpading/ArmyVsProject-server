...
router.post('/', asyncHandler( async (req, res) => {
    ...
    if(!isSame) throw new BadRequest(ERROR_CODE.TYPE_MISMATCH, 'selectedElemId, unselectedElemId must have same type')
...
//특정 질문에 대한 답변 통계 반환
router.get('/stat/:elemId1/:elemId2', asyncHandler( async (req, res) => {
    ...
    const { elemType1 , elemType2, selectedElemCount, unselectedElemCount } = selectElemCountResult.rows[0]

    if (elemType1 == null || elemType2 == null) throw new NotFound()
    if (elemType1 != elemType2) throw new BadRequest(ERROR_CODE.TYPE_MISMATCH, 'selectedElemId, unselectedElemId must have same type')
